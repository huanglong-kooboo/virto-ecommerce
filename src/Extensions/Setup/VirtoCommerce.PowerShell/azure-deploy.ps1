...
Function Build
{
    if($build)
    {
        ...
        & "$global:buildexe_path\MSBuild.exe" $build_solutionname /m /t:clean $build_params /p:Configuration="$build_configuration" /p:VisualStudioVersion=12.0
        if ($LASTEXITCODE -ne 0)
        {
          throw "Build Failed"
        }
        ...
        & "$global:buildexe_path\MSBuild.exe" $build_solutionname /m /t:build /p:Configuration="$build_configuration" /Property:ApplicationVersion=$PublishApplicationVersion /p:VisualStudioVersion=12.0 /Property:MinimumRequiredVersion=$PublishApplicationVersion $build_params
        if ($LASTEXITCODE -ne 0)
        {
          throw "Build Failed"
        }
        ...
        & "$global:buildexe_path\MSBuild.exe" $build_solutionname /m /t:Publish /p:InstallUrl=$admin_installcontainer/$admin_blobprefix/ /p:Configuration="$build_configuration" /p:TargetProfile=Cloud /p:VisualStudioVersion=12.0 /Property:PublishDir="$build_path\" /Property:ApplicationVersion=$PublishApplicationVersion /Property:MinimumRequiredVersion=$PublishApplicationVersion $build_params
        if ($LASTEXITCODE -ne 0)
        {
          throw "Build Failed"
        }
...
Function build-search
{
    ...
    & "$global:buildexe_path\MSBuild.exe" $search_workerrolehome\ESWorkerRole.csproj /p:Configuration=$build_configuration /p:VisualStudioVersion=12.0
    if ($LASTEXITCODE -ne 0)
    {
       throw "Deployment failed"
    }
